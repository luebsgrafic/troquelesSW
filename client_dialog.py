...
import sqlite3
<<<<<<< HEAD
from PySide6.QtWidgets import (QDialog, QVBoxLayout, QTableView, QPushButton, QHBoxLayout, QHeaderView, QMessageBox, QDialogButtonBox, QFormLayout, QLineEdit, QDoubleSpinBox)
...
# NUEVO: Importamos la nueva ventana de presupuestos
...
<<<<<<< HEAD
        self.setWindowTitle("Gestión de Clientes"); self.setMinimumSize(900, 600)
        ...
        self.tabla_clientes = QTableView(); self.tabla_clientes.setModel(self.modelo_clientes)
        self.tabla_clientes.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch); self.tabla_clientes.hideColumn(0)
        self.tabla_clientes.setSelectionMode(QTableView.SelectionMode.SingleSelection); self.tabla_clientes.setSelectionBehavior(QTableView.SelectionBehavior.SelectRows)
        layout.addWidget(self.tabla_clientes)
        botones_layout = QHBoxLayout(); botones_layout.setAlignment(Qt.AlignmentFlag.AlignLeft)
        btn_anadir = QPushButton("Añadir Cliente"); self.btn_ver_presupuestos = QPushButton("Ver Presupuestos"); self.btn_ver_presupuestos.setEnabled(False)
        botones_layout.addWidget(btn_anadir); botones_layout.addWidget(self.btn_ver_presupuestos); layout.addLayout(botones_layout)
        btn_anadir.clicked.connect(self.abrir_dialogo_anadir_cliente); self.btn_ver_presupuestos.clicked.connect(self.abrir_dialogo_presupuestos)
        self.tabla_clientes.selectionModel().selectionChanged.connect(self.actualizar_estado_botones)

    def crear_modelo_tabla(self):
        modelo = QSqlTableModel(self, self.db); modelo.setTable("Clientes"); modelo.select()
    ...
    def abrir_dialogo_anadir_cliente(self):
        dialogo = QDialog(self); dialogo.setWindowTitle("Añadir Nuevo Cliente"); layout = QFormLayout(dialogo)
        input_nombre = QLineEdit(); input_empresa = QLineEdit(); input_cif = QLineEdit(); input_direccion = QLineEdit(); input_telefono = QLineEdit(); input_email = QLineEdit()
        input_beneficio = QDoubleSpinBox(); input_beneficio.setDecimals(2); input_beneficio.setSuffix(" %"); input_beneficio.setValue(25.0)
        layout.addRow("Nombre:", input_nombre); layout.addRow("Empresa:", input_empresa); layout.addRow("CIF:", input_cif); layout.addRow("Dirección:", input_direccion); layout.addRow("Teléfono:", input_telefono); layout.addRow("Email:", input_email); layout.addRow("Beneficio por defecto:", input_beneficio)
        ...
        if dialogo.exec():
            ...
            if not nombre: QMessageBox.warning(self, "Campo Obligatorio", "El nombre no puede estar vacío."); return
            conn = sqlite3.connect("troquelgest.db"); cursor = conn.cursor()
            try:
                cursor.execute("INSERT INTO Clientes (nombre, empresa, cif, direccion, telefono, email, porcentaje_beneficio_defecto) VALUES (?, ?, ?, ?, ?, ?, ?)", (nombre, input_empresa.text(), input_cif.text(), input_direccion.text(), input_telefono.text(), input_email.text(), input_beneficio.value()))
                conn.commit(); self.modelo_clientes.select()
            except sqlite3.Error as e: QMessageBox.critical(self, "Error", f"{e}")
            finally: conn.close()

    def actualizar_estado_botones(self):
        self.btn_ver_presupuestos.setEnabled(self.tabla_clientes.selectionModel().hasSelection())

    def abrir_dialogo_presupuestos(self):
        indices = self.tabla_clientes.selectionModel().selectedRows()
        if not indices: return
        fila = indices[0].row()
        id_cliente = self.modelo_clientes.record(fila).value("id")
        nombre_cliente = self.modelo_clientes.record(fila).value("nombre")
        dialogo = PresupuestoDialog(id_cliente, nombre_cliente, self); dialogo.exec()
=======
        ...
        # NUEVO: Permitir seleccionar solo una fila a la vez
        ...
        self.btn_ver_presupuestos = QPushButton("Ver Presupuestos") # NUEVO
        self.btn_ver_presupuestos.setEnabled(False) # NUEVO: Deshabilitado por defecto

        botones_layout.addWidget(btn_anadir)
        botones_layout.addWidget(self.btn_ver_presupuestos) # NUEVO
        layout.addLayout(botones_layout)

        # Conexiones de los botones
        btn_anadir.clicked.connect(self.abrir_dialogo_anadir_cliente)
        self.btn_ver_presupuestos.clicked.connect(self.abrir_dialogo_presupuestos) # NUEVO

        # NUEVO: Conectar la selección de la tabla a una función
        self.tabla_clientes.selectionModel().selectionChanged.connect(self.actualizar_estado_botones)
    ...
    def abrir_dialogo_anadir_cliente(self):
        ...
        botones.accepted.connect(dialogo.accept); botones.rejected.connect(dialogo.reject)
        ...
        if dialogo.exec():
            ...
            try:
                ...
                QMessageBox.critical(self, "Error de Base de Datos", f"No se pudo guardar el cliente. ¿Quizás el CIF ya existe?\n\n{e}")
            finally:
                conn.close()
    ...
    def actualizar_estado_botones(self):
        ...
        # .hasSelection() devuelve True si hay una o más filas seleccionadas
    ...
    def abrir_dialogo_presupuestos(self):
        ...
        # El índice 0 es el ID (aunque esté oculto), el 1 es el Nombre
