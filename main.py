...
def init_database():
    """Función para crear y abrir la conexión a la base de datos."""
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Crear widget central con botón principal
        widget_central = QWidget()
        self.setCentralWidget(widget_central)

        layout_principal = QVBoxLayout(widget_central)

        # Botón principal para crear presupuesto
        self.btn_nuevo_presupuesto = QPushButton("➕ Crear Nuevo Presupuesto")
        font = self.btn_nuevo_presupuesto.font()
        font.setPointSize(24)
        self.btn_nuevo_presupuesto.setFont(font)
        self.btn_nuevo_presupuesto.setMinimumHeight(100)
    ...
    def iniciar_proceso_presupuesto(self):
        """Inicia el proceso de creación de un nuevo presupuesto."""
        # Primero seleccionar cliente
        select_dialog = SelectClientDialog(self)
        if select_dialog.exec():
            # Luego crear presupuesto para el cliente seleccionado
            presupuesto_dialog = NewPresupuestoDialog(
                select_dialog.selected_client_id,
                select_dialog.selected_client_name,
                self
            )
            presupuesto_dialog.exec()

    def abrir_dialogo_configuracion(self):
        """Abre el diálogo de configuración de costes."""
...
if __name__ == '__main__':
    ...
    # Intentamos conectar a la BD antes de mostrar nada
